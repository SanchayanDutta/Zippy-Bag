"""
Exact information-theoretic oracle for the 200-object k-ary OQA dataset.

This module implements the *exact* dynamic program used for the Oracle
(Optimal) line in the k-ary 200-objects entropy plot.
...
1. Load the 300-object JSON file into a Python dictionary:
       ...
       import json
       with open("data/kary200_Objects.json", "r") as f:
           items = json.load(f)
...
class KaryOracleDP:
    ...
    items:
        ...
        ``kary200_Objects.json``.  Every object must have the same set of
...
def _try_demo_with_local_json():
    """If run as a script from the dataset root, load the 200-object table and
    ...
    if not json_path.exists():
        print("[oracle demo] kary200_Objects.json not found; "
